...
- [PART 2 — KUBECTL NINJA](#part-2---kubectl-ninja)
    - [SECTION A — kubectl Essentials (But Expert-Grade)](#section-a---kubectl-essentials-but-expert-grade)
        - [A.1 Use kubectl like a CLI Ninja](#a1-use-kubectl-like-a-cli-ninja)
        - [A.2 Context/namespace switching](#a2-contextnamespace-switching)
    - [SECTION B — Brutal Debugging & Inspection](#section-b---brutal-debugging--inspection)
        - [B.1 Describe EVERYTHING](#b1-describe-everything)
        - [B.2 Logs — expert mode](#b2-logs--expert-mode)
        - [B.3 Exec inside pods](#b3-exec-inside-pods)
        - [B.4 Debug a pod using ephemeral container](#b4-debug-a-pod-using-ephemeral-container)
    - [SECTION C — Creation, Apply, Replace, Force](#section-c---creation-apply-replace-force)
        - [C.1 Create and edit objects](#c1-create-and-edit-objects)
        - [C.2 Replace (idempotent + powerful)](#c2-replace-idempotent--powerful)
        - [C.3 Force replace (useful when fields are immutable)](#c3-force-replace-useful-when-fields-are-immutable)
        - [C.4 Patch (the ultimate DevOps weapon)](#c4-patch-the-ultimate-devops-weapon)
    - [SECTION D — Rollouts, Restarts, History](#section-d---rollouts-restarts-history)
        - [D.1 Restart](#d1-restart)
        - [D.2 History](#d2-history)
    - [SECTION E — Node-Level Operations (Advanced)](#section-e---node-level-operations-advanced)
        - [E.1 Cordon & drain](#e1-cordon--drain)
        - [E.2 Uncordon](#e2-uncordon)
        - [E.3 Check kubelet](#e3-check-kubelet)
        - [E.4 Restart kubelet](#e4-restart-kubelet)
    - [SECTION F — Static Pods (Where DevOps Magic Lives)](#section-f---static-pods-where-devops-magic-lives)
        - [F.1 Force rebuild control plane pods](#f1-force-rebuild-control-plane-pods)
    - [SECTION G — Certificates Ninja Mode](#section-g---certificates-ninja-mode)
        - [G.1 View certificate expiration](#g1-view-certificate-expiration)
        - [G.2 Renew ALL certs](#g2-renew-all-certs)
        - [G.3 Regenerate apiserver cert with new IP or domain](#g3-regenerate-apiserver-cert-with-new-ip-or-domain)
    - [SECTION H — Stuck Pods, PVCs, Finalizers](#section-h---stuck-podspvcs-finalizers)
        - [H.1 Force delete terminating pod](#h1-force-delete-terminating-pod)
        - [H.2 Remove finalizers](#h2-remove-finalizers)
        - [H.3 Delete stuck namespace](#h3-delete-stuck-namespace)
    - [SECTION I — Debugging Networking, DNS, CNI](#section-i---debugging-networking-dns-cni)
        - [I.1 DNS test](#i1-dns-test)
        - [I.2 Network test](#i2-network-test)
        - [I.3 Check CNI](#i3-check-cni)
        - [I.4 Node local routing](#i4-node-local-routing)
    - [SECTION J — ETCD Debugging via kubectl](#section-j---etcd-debugging-via-kubectl)
        - [Check etcd pod](#check-etcd-pod)
        - [Exec into etcd pod](#exec-into-etcd-pod)
        - [ETCD endpoint health](#etcd-endpoint-health)
    - [SECTION K — Cluster-Wide Searches](#section-k---cluster-wide-searches)
        - [K.1 Search for all pods with label](#k1-search-for-all-pods-with-label)
        - [K.2 Field selectors](#k2-field-selectors)
        - [K.3 Grep pods by name](#k3-grep-pods-by-name)
    - [SECTION L — YAML Ninja Mode](#section-l---yaml-ninja-mode)
        - [L.1 Output YAML for any object](#l1-output-yaml-for-any-object)
        - [L.2 Dry-run to generate YAML](#l2-dry-run-to-generate-yaml)
    - [SECTION M — Exec on Nodes (DaemonSets for emergency)](#section-m---exec-on-nodes-daemonsets-for-emergency)
    - [SECTION N — Fixing Common Control Plane Issues](#section-n---fixing-common-control-plane-issues)
        - [N.1 API server is down](#n1-api-server-is-down)
        - [N.2 kubelet not pulling static pods](#n2-kubelet-not-pulling-static-pods)
        - [N.3 etcd corruption](#n3-etcd-corruption)
    - [SECTION O — Best Commands for Daily Operations](#section-o---best-commands-for-daily-operations)
        - [Create namespace](#create-namespace)
        - [Label nodes](#label-nodes)
        - [Taint nodes](#taint-nodes)
        - [Remove taint](#remove-taint)
        - [Run temporary pod](#run-temporary-pod)
        - [Show all events](#show-all-events)
        - [Edit live config](#edit-live-config)
    - [Summary](#summary)
    - [Question: does this cause restart?](#question-does-this-cause-restart)
